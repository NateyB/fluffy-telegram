 ...
 * Q learning
 */
public class SarsaLambdaPlayer implements TicTacToePlayer
{
    ...
    @Override public void giveReward(Board board, double reward, boolean terminal)
    {
        ...
        double delta = reward + 0.9 * qTable.getQValue(board, actionToTake) - qTable.getQValue(prevBoard, prevAction);
        ...
        {
            qTable.setQValue(tuple, qTable.getQValue(tuple) + qTable.alphaCalc(tuple) * delta * qTable.getEligibility(tuple));
            qTable.setEligibility(tuple, .9 * .9 * qTable.getEligibility(tuple));
        }
