 ...
 * Q learning
 */
public class SarsaPlayer implements TicTacToePlayer
{
    ...
    {
        return Collections.max(board.getAvailableActions(), Comparator.comparingDouble(a -> qTable.getQValue(board, a)));
    }

    int getNextAction(Board board)
    {
        return board.getAvailableActions().size() == 0 ? -1 : (Math.random() < eps ? board.getAvailableActions().get(new Random().nextInt(board.getAvailableActions().size())) : getBestAction(board));
    }
    ...
    @Override public void giveReward(Board board, double reward, boolean terminal)
    {
        ...
        qTable.setQValue(prevBoard, prevAction, qTable.getQValue(prevBoard, prevAction) + (0.9 * qTable.alphaCalc(prevBoard, prevAction) * (reward + 0.9 * qTable.getQValue(board, actionToTake) - qTable.getQValue(
                prevBoard, prevAction))));
