...
import com.natebeckemeyer.school.projects.ml.reinforcement.QLearningLambdaPlayer;
import com.samvbeckmann.machinelearning.reinforcement.players.*;
...
class Environment {
    ...
    public static void main(String[] args) {
        ...
        xPlayer = new QLearningLambdaPlayer();
        oPlayer = new RandomPlayer();
    ...
    private void runSimulation() {
        if (stats)
            System.out.printf("Rounds\tx%%\to%%\tt%%%n");
        for (int j = 0; j < 10000/rounds; j++) {
            for (int i = 0; i < rounds; i++) {
                winArray[i] = runGame(new MinimaxTicTacToePlayer());
                completedRounds++;
            }
            if (stats) {
                System.out.printf("%d\t",completedRounds);
    ...
    private String getStatsString() {
        ...
        result.append(String.format("%5f\t", (double) xWins/rounds));
        result.append(String.format("%5f\t", (double) oWins/rounds));
        result.append(String.format("%5f", (double) ties/rounds));
